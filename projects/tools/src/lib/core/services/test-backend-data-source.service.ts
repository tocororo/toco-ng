...
import { sortData } from './backend-data-source';
...
export class UserService
{
    page(request: PageRequest/*<User>*/, query: UserQuery): Observable<Page1<User>>
    {
        ...
        if (search)
        {
            ...
            filteredUsers = filteredUsers.filter(
                ...
                ({ id, name, registrationDate }) =>
                    id.toString(10).includes(search) ||
        ...
        {
            while ((start = request.page * request.size) >= filteredUsers.length) request.page--;
        }
        else
        {
            request.page = 0;
        ...
        const end: number = start + request.size;
        ...
        const page = {
            content: pageUsers,
            number: request.page,
            size: request.size/*pageUsers.length*/,
